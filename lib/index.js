    ...
    constructor(databasePath) {
        ...
        this.lookupCollectionData = (collectionName, lookupValue) => {
            if (typeof lookupValue === 'string') {
                return new utils_1.Ok(this.collectionStore[collectionName][lookupValue]);
            }
        ...
        this.insert = (collectionName, key, storageObject) => {
        ...
        this.update = (collectionName, lookupValue, storageObject) => {
            const foundKey = this.lookupCollectionData(collectionName, lookupValue);
            if (!foundKey.ok)
        ...
        this.replace = (collectionName, lookupValue, storageObject) => {
            const foundKey = this.lookupCollectionData(collectionName, lookupValue);
            if (!foundKey.ok)
                return new utils_1.Err(foundKey.val);
            if (!foundKey.val)
